...
<body style="background-image: url('../lightbg.png')">
    ...
    if (!isset($_GET["new"])) $_GET["new"] = "no";
    if (isset($_POST["paste"])) {
        $paste = $_POST["paste"];
    } elseif (!($_GET["new"] == "yes")) {
        $paste = file_get_contents($rootdir . "trash/" . $_GET["filename"]);
        if (!$paste) $paste = "COULDN'T LOAD EXISTING CONTENTS, SAVING THIS WILL OVERWRITE!!";
    }
    ...
    if (isset($_GET["invalidfile"]) && $_GET["invalidfile"] == "yes") {
